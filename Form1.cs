...
namespace Praktikum_AD_Week_10
{
    public partial class HasilPertandingan : Form
    {
        ...
        private void btnCheck_Click(object sender, EventArgs e)
        {
            ...
            sqlQuery = "select dm1.minute as Minute, if (dm1.type = 'GW', if(dm1.team_id != m.team_home, p.player_name,''), if (p.team_id = m.team_home, p.player_name, '' )) as 'Player Name 1', if (dm1.type = 'GW', if(dm1.team_id != m.team_home, 'Own Goal' ,''), if(p.team_id = m.team_home, if(dm1.type = 'CY', 'Yellow Card', if(dm1.type = 'CR', 'Red Card', if(dm1.type = 'GO', 'Goal', if(dm1.type = 'GP', 'Goal Penalty', if(dm1.type = 'GW', 'Own Goal', if(dm1.type = 'PM', 'Penalty Miss', p.player_name)))))), '')) as 'Tipe 1', if (dm1.type = 'GW', if(dm1.team_id != m.team_away, p.player_name,''), if (p.team_id = m.team_away, p.player_name, '' )) as 'Player Name 2', if (dm1.type = 'GW', if(dm1.team_id != m.team_away, 'Own Goal' ,''), if(p.team_id = m.team_away, if(dm1.type = 'CY', 'Yellow Card', if(dm1.type = 'CR', 'Red Card', if(dm1.type = 'GO', 'Goal', if(dm1.type = 'GP', 'Goal Penalty', if(dm1.type = 'GW', 'Own Goal', if(dm1.type = 'PM', 'Penalty Miss', p.player_name)))))), '')) as 'Tipe 2' from dmatch dm1, player p, `match` m where dm1.match_id = m.match_id and p.player_id = dm1.player_id and m.team_home = '" + cbBoxLeft.SelectedValue.ToString() + "' and m.team_away = '" + cbBoxRight.SelectedValue.ToString() + "' order by 1";
            sqlCommand = new MySqlCommand(sqlQuery, sqlConnect);
            sqlAdapter = new MySqlDataAdapter(sqlCommand);
